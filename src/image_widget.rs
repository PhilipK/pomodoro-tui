/// Adapted from : https://github.com/arraypad/tui-image
/// did not work with newest tui-rs version, and did not have the percent option.
...
impl<'a> Image<'a> {
    ...
    pub fn percent(mut self, percent: Option<f32>) -> Image<'a> {
    ...
    fn draw_img(&self, area: Rect, buf: &mut Buffer, img: &RgbaImage) {
        ...
        // draw
        let height = oy + min((img.height() / 2) as u16, area.height - 1);
        let width = min(ox + img.width() as u16, area.width - 1);
        let total_area = height as f32 * width as f32;
        let max_area = match self.percent {
            Some(p) => p,
            None => 1.0,
        } * total_area;
        let mut i = 0;
